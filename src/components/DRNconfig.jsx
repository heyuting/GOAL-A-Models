...
export default function DRNConfig({ savedData }) {
  ...
  const submitFullPipeline = async () => {
    ...
    try {
      ...
      // Convert timeStep to number if it's a string (from input field)
      const timeStepNum = typeof timeStep === 'string' ? parseFloat(timeStep) : Number(timeStep);
      if (!timeStepNum || timeStepNum <= 0 || !Number.isFinite(timeStepNum)) {
      ...
      const pipelineData = {
        ...
        time_step: timeStepNum, // Use the validated numeric value
  ...
  const checkFullPipelineStatus = async (jobId) => {
    ...
    try {
      ...
      if (!responseText || responseText.trim() === '') {
        throw new Error('Empty response from server. Backend may be experiencing issues.');
      }
      ...
      if (!result.status) {
        console.warn('Response missing status field:', result);
        throw new Error('Backend response missing status field');
      }
            ...
            <div className="mt-16 space-y-3">
              ...
              {fullPipelineStatus === 'completed' && (
                <Button
                  ...
                  className={`w-full text-lg font-semibold rounded-md p-8 ${isDownloading
            ...
            <CardContent>
              ...
              <div className="flex items-stretch mb-6 pb-4 border-b">
                ...
                <button
                  ...
                  className={`flex-1 px-3 py-1 h-12 text-sm font-medium transition-colors rounded-r-sm h-9 ${savedData || (locationMode === 'single' && selectedLocations.length >= 1) ||
                    (locationMode === 'multiple' && selectedLocations.length >= 2 && outletCheckStatus === 'same')
                    ? currentPage === 2
                          ...
                          <div className="bg-gray-50 p-3 rounded-lg border">
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-1 rounded text-xs font-medium ${fullPipelineStatus === 'completed' ? 'bg-green-100 text-green-700' :
                            ...
                            <div className="text-xs text-gray-600 space-y-1">
                              ...
                              <div>
                                <strong>Locations:</strong> {selectedLocations.length} location(s)
                                {selectedLocations.length > 0 && (
                                  <div className="mt-1 ml-0 pl-0 space-y-0.5">
                                    {selectedLocations.map((loc, idx) => (
                                      <div key={idx} className="text-gray-600">
                                        {selectedLocations.length > 1 ? `Location ${idx + 1}: ` : ''}
                                        ({typeof loc.lat === 'number' ? loc.lat.toFixed(4) : loc.lat}, {typeof loc.lng === 'number' ? loc.lng.toFixed(4) : loc.lng})
                                      </div>
                                    ))}
                                  </div>
                                )}
                                      ...
                                      <div className="flex gap-1">
                                        {[1, 2, 3, 4, 5].map((stepNum) => {
                                          ...
                                          return (
                                            <div
                                              key={stepNum}
                                              className={`flex-1 h-2 rounded ${stepNum < stepProgress.step
                                                ? 'bg-green-500'
